...
const StylistListScreen = () => {
  ...
  const getFetch = () => {
    let url =
      "http://localhost/mobile-salon-app/backend/constrollers/StylistApiController.php";
    if (city) {
      ...
      <div className="row">
        <div className="col-2">
          <select onChange={(e) => setCity(e.target.value)}>
            ...
            {stylistList &&
              showAvailableCities(stylistList).map((item, index) => (
          ...
          {stylistList &&
            showAvailableServices(stylistList).map((item, index) => (
              <label key={index}>
          ...
          <div className="row">
            {stylistList &&
              stylistList.map((stylist, index) => (
                <div className="col-2">
                  <StylistBox key={index} stylist={stylist && stylist} />
                </div>
              ))}
          </div>
        </div>
